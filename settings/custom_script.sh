...
goto_install() {
cd $MODPATH/bin/Magisk/$mymagiskinstall
...
cd ..
mv ./$ABI/* ./
cp -af $ABI32/libmagisk.so ../magisk32 2>/dev/null
chmod -R 755 .
install_magisk
sleep 2
}
...
ddbootpatch() {
    if [[ $SELECT_OUTPUT = "APatch" ]]; then
        ...
        dd if=boot.img of="$position" bs=4M
        ddcleanup
    elif [[ $SELECT_OUTPUT = "SukiSU_Ultra" ]]; then
        if [[ `echo "5.1 > $KERNEL_VER" | bc` -eq 1 ]]; then
            Aurora_abort "你的内核版本不太适合SukiSU_Ultra呢，或许你需要自己编译"
        fi
        if [[ ! -d /data/data/com.sukisu.ultra ]]; then
            Aurora_abort "在转换之前，请先安装SukiSU_Ultra管理器！"
        fi
        PATCH_PATH="$MODPATH/bin/SUKISU/"
        foundboot
        gkiforksu "sukisukernel.conf" "sukisu_url.conf"
        mv -f new-boot.img boot.img
        sleep 2
        ./bin/magiskboot cleanup
        dd if=boot.img of="$position" bs=4M
...
if [[ -z "$APATCH" ]] && [[ -z "$KSU" ]] && [[ -n "$MAGISK_VER_CODE" ]]; then
    ...
    not_magisk="ksu"
    if [[ -d /data/data/com.sukisu.ultra ]]; then
        Aurora_ui_print "您正在使用SukiSU_Ultra($VERSION)"
        whichroot="SukiSU_Ultra"
        newest_version="$(sed -n '8p' $MODPATH/bin/newest_version.conf)"
        return
    elif [[ -d /data/data/com.rifsxd.ksunext ]]; then
